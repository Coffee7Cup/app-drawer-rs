...
pub fn app_icon() -> Container {
    ...
    container()
        .width(ICON_DIM) // ← shrink wrapper to actual icon width, no phantom space
        .height(ICON_DIM)
        ...
        .transform(move || {
            ...
            let s = if let Some(x) = distance.get() {
                if x > 0.0 && x < 95.0 {
                    ((max - min) / pad) * x + min
                } else if (95.0..=185.0).contains(&x) {
                    max
                } else {
            ...
            // Scale from bottom-center: cx = half icon width, cy = full icon height
            Transform::scale(s).center_at(ICON_DIM / 2.0, ICON_DIM)
        })
        ...
        .children([image("./default-icon.png")
            .width(ICON_DIM as u32)
            .height(ICON_DIM as u32)
